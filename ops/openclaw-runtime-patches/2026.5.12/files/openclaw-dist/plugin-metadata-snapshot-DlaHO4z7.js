...
const PLUGIN_METADATA_SNAPSHOT_CACHE = /* @__PURE__ */ new Map();
function resolvePluginMetadataSnapshotCacheKey(params) {
	...
	return hashJson({
		config: params.config ?? {},
		discovery: fingerprintPluginDiscoveryContext(discovery),
		hasIndex: params.index !== void 0,
		indexFingerprint: params.index ? resolveInstalledManifestRegistryIndexFingerprint(params.index) : null,
...
function loadPluginMetadataSnapshot(params) {
	...
	const cached = PLUGIN_METADATA_SNAPSHOT_CACHE.get(cacheKey);
	if (cached) return cached;
	return measureDiagnosticsTimelineSpanSync("plugins.metadata.scan", () => {
		const snapshot = loadPluginMetadataSnapshotImpl(params);
		PLUGIN_METADATA_SNAPSHOT_CACHE.set(cacheKey, snapshot);
		return snapshot;
	}, {
