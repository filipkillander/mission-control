...
async function sessionsCommand(opts, runtime) {
	...
	const configContextTokens = configuredContextTokens ?? 2e5;
		...
		writeRuntimeJson(runtime, {
			...
			sessions: await Promise.all(rows.map(async (row) => {
				...
				return {
					...
					contextTokens: r.contextTokens ?? configuredContextTokens ?? configContextTokens ?? null,
					modelProvider: modelRef.provider,
					model: modelRef.model
				};
			}))
	...
	for (const row of rows) {
		...
		const contextTokens = row.contextTokens ?? configuredContextTokens ?? configContextTokens;
