...
const CLI_PROVIDER_RESULT_CACHE = /* @__PURE__ */ new Map();
function isCliProvider(provider, cfg) {
	const normalized = normalizeProviderId(provider);
	if (CLI_PROVIDER_RESULT_CACHE.has(normalized)) return CLI_PROVIDER_RESULT_CACHE.get(normalized) === true;
	const backends = cfg?.agents?.defaults?.cliBackends ?? {};
	const result = Object.keys(backends).some((key) => normalizeProviderId(key) === normalized) || resolveRuntimeCliBackends().some((backend) => normalizeProviderId(backend.id) === normalized) || Boolean(resolvePluginSetupCliBackendRuntime({
		backend: normalized,
		config: cfg
	}));
	CLI_PROVIDER_RESULT_CACHE.set(normalized, result);
	return result;
}
//#endregion
